...
if figures_save
    cd([homepath,'figures']);
    figure(2);
    saveas(gcf,'Crane_surfaceElevationSpeeds.png');
    disp('figure 2 saved');
end
...
figure(4); clf % Terminus positions along centerline
    ...
    grid on; %axis equal
figure(5); clf % Terminus positions over time
    hold on;
...
for i=1:length(centerline.termx)
    ...
    if mod(i-1,10) == 0 % Only include every 10 in legend
        figure(4); hold on; plot(termx,termy,'.','color',col(i,:),'markersize',30,'displayname',num2str(termDate));
        figure(5); hold on; plot(termDate,x(xn),'.','color',col(i,:),'markersize',30);
    else
        figure(4); hold on; plot(termx,termy,'.','color',col(i,:),'markersize',30,'handlevisibility','off');
        figure(5); hold on; plot(termDate,x(xn),'.','color',col(i,:),'markersize',30);
    end
