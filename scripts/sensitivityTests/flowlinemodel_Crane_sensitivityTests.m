...
delta_fwd0 = 0:0.01:0.1; % m change in fwd
...
for j=1%:length(delta_smb0)
    ...
    %try
        ...
        % plot results
        if t(i)==t_end
            h2=h; H2=H; x2=x; c2=c; gl2=gl; U2=U; fwd2=fwd; Fgl2=Fgl; % store final geometry & speed
        ...
        if save_final
            ...
            if delta_fwd==0 && delta_smr==0 && delta_smb==0 
                save(['SMR0_SMB0_geom.mat'],'h2','H2','c2','c2','U2','gl2','x2','fwd2','Fgl2');
            elseif delta_fwd==0 
                fileName = ['SMR',num2str(delta_smr),'_SMB',num2str(delta_smb),'_geom.mat'];
                save(fileName,'h2','H2','c2','c2','U2','gl2','x2','fwd2','Fgl2');
            else
