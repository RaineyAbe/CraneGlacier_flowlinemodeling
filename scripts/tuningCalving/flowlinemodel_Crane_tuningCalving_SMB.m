...
% calving parameters
    ...
    fwd = 30; %10:10:100; % fresh water depth in crevasses (m)
...
%    for q=1:length(fwd)  
        ...
        %try
               ...
               if tune_calving
                ...
                elseif mod(i-1,round(length(t)/10))==0
                    ...
                    figure(3); hold on; % Plot terminus position every 10 time iterations                       
                        plot(x(c)./10^3,t(i)./3.1536e7+2009,'.','Color',col(i,:),'markersize',15,'linewidth',1.5,'displayname',num2str(t(i)./3.1536e7)); hold on;
                end  
            ...
            %A = interp1(x0,A0_adj(yr,:),xn,'linear','extrap'); % rate factor (Pa^-n s^-1)
            A = polyval(polyfit(x0,A0_adj(yr,:),1),x);
