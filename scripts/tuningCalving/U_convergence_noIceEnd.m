...
function [U,dUdx,vm,T] = U_convergence_noIceEnd(x,U,U0,dUdx,dhdx,H,A,E,N,W,dx,c,n,m,beta,rho_i,rho_sw,g)
...
while b
   ...
   for k=1:c
        ...
        elseif k==c 
            %set the calving front coefficients
            G_minus(k) = -1;
            G(k) = 1;
            G_plus(k) = 0;  
   ...
   % use driving stress at the upper boundary that results in 90% observed
    ...
    % calving front velocity condition
    Un(c+1:length(x)) = zeros(1,length(x(c+1:end)));
