...
% regrid the initialization data to match the desired grid spacing, 
    ...
    % find the location of the grounding line
...
% calving parameters
    Hc = 400; %100:100:1000; % m -> set the calving front to a default minimum ice thickness value
    fwd = 5:5:100; % fresh water depth in crevasses (m)
    ...
    for q=1:length(fwd)  
        ...
        try
            for i=1:length(t)
                ...
                for k=1:length(x) % thickness can't go beneath bed elevation
                    if H(k) >= h(k)-hb(k)
                        H(k) = h(k)-hb(k);
                    end
                end    
                ...
                h(c+1:length(x)) = zeros(1,length(x(c+1:end))); % zeros past calving front
                ...
                c = find(h(1:end-1)-crev(1:end-1)<=0,1,'first'); %calving front located where the inland-most crevasse intersects sea level                
                ...
                [U,dUdx,vm,T] = U_convergence_noIceEnd(x,U,U0,dUdx,dhdx,H,A,E,N,W,dx,c,n,m,beta,rho_i,rho_sw,g); 
                ...
                Hn(c+1:end) = zeros(1,length(x(c+1:end))); % remove thickness values past calving front
                H = Hn; %set as the new thickness value
                for k=1:length(H) % thickness cannot go beneath bed
                    if H(k)>h(k)-hb(k)
                        H(k)=h(k)-hb(k);
                    end
                end
                ...
                H = interp1(x,H,xn,'linear','extrap'); %ice thickness (m)
                    H(c+1:end) = zeros(1,length(H(c+1:end))); 
                ...
                %find the location of the grounding line for the adjusted data
                gl = find(Hf-H>0,1,'first')-1; %grounding line location   
                ...
                %h(c+1:length(x)) = zeros(1,length(x(c+1:end)));
            ...
            break; % continue to the next loop
        end
...
colorbar; set(gca,'fontsize',14,'linewidth',2);
xlabel('fwd (m)'); ylabel('Hc (m)');
