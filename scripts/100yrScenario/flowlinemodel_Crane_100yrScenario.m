...
%   4. Tune the backstress sigma_b using the resistance stress term to 
...
% regrid the initialization data to match the desired grid spacing, 
    ...
    c = dsearchn(xi',term_obs(1)); % 2009 terminus location (index)
    ...
    ice_end=ice_endi; E = ones(1,length(x)); % enhancement factor
...
%   conditions using 2009 observations
 ...
 % Run flowline model
    for i=1:length(t)
        ...
        %U(1:ice_end) = movmean(U(1:ice_end),10);
    ...
    save('Crane_flowline_100yr_output3.mat','xj','hj','Wj','Hj','Aj',...
...
for f=1:length(F0)
    ...
    for i=1:length(t)
        ...
        % find the calving front location (based on Benn et al., 2007 & Nick et al., 2010)
        Rxx = 2*nthroot((dUdx./(E.*A)),n)-sigma_b; %resistive stress (Pa)
        crev = (Rxx./(rho_i.*g))+((rho_fw./rho_i).*fwd); %crevasse penetration depth (m)
        c = find(h(1:ice_end-1)-crev(1:ice_end-1)<=0,1,'first'); %calving front located where the inland-most crevasse intersects sea level
        %if the crevasses never intersect sea level
        if isempty(c) == 1
...
% define sigma_b values to test
    sigma_b = [50e3:50e3:300e3]; % back pressure (Pa)
...
for f=1:length(sigma_b)
    ...
    for i=1:length(t)
        ...
        % Calculate calving front misfit each full year
...
%% 4. Tune enhancement factor E (via h & U)
...
for f=1:length(Efit)
    ...
    for i=1:length(t)
        ...
        % U is non-zero everywhere - check for grid error at ice_end
        if U(ice_end)==0
            U(ice_end) = U(ice_end-1);
        end
...
fwd = 30:1:40; % fresh water depth in crevasses (m)
...
% save results
cd([homepath,'inputs-outputs']);
fwdbest = fwd(misfit==min(misfit)); % m.w.e.
...
fwd = load('fwdbest.mat').fwdbest; % optimal fresh water depth from step #4
F0 = load('F0best.mat').F0best; % optimal inner boundary flux from step #5
