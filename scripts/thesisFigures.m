...
ax4=axes('Position',[0.55 0.1 0.35 0.35],'linewidth',2,'fontsize',fontsize,'fontname',font); % speed
    ...
    c = colorbar('Limits',[2002 2019],'Ticks',[2002 2010 2019],'Position',[.92 .32 .03 .3410],...
        'fontsize',fontsize);
    for i=1:length(SMB)
        plot(ax4,cl.xi./10^3,SMB(i).smb_interp,'color',col(col_SMB(i),:),'linewidth',linewidth);
    end
    % Add text label
    text(55,(max(get(gca,'YLim'))-min(get(gca,'YLim')))*0.925+min(get(gca,'YLim')),...
...
ax2=axes; set(gca,'position',[0.58 0.58 0.4 0.4]);
    ...
    annotation('textarrow',[0.6 0.6],[0.6 0.6],'string','}', ...
       ...
       'FontSize',28,'FontName',fontname,'TextColor','k','fontweight','bold');
ax3=axes; set(gca,'position',[0.08 0.08 0.83 0.4]);
    ...
    text(43.2,(max(get(gca,'YLim'))-min(get(gca,'YLim')))*0.925+min(get(gca,'YLim')),...
        'c)','fontsize',fontsize,'linewidth',1,'backgroundcolor','w');     
if save_figure
    figure(3);
...
cd([homepath,'inputs-outputs']);
load('Crane_2100_noChange_runoff.mat');
...
smb0 = load('Crane_flowlineModelInitialization.mat').smb0;
% load initial fwd
fwd0 = load('optimalfwd.mat').optfwd;

cd([homepath,'scripts/modelingWorkflow/results_runoff']);
addpath([homepath,'../matlabFunctions/cmocean_v2.0/cmocean']);
...
colF = cmocean('rain',4); % color scheme for discharge plot
...
while loop==1
    ...
    ax2=axes('position',[0.405 0.73 0.25 0.25]); hold on; grid on; % b) speed
        set(gca,'fontsize',fontsize,'linewidth',2,'YTick',0:1000:5000);        
        ylabel('U (m a^{-1})');  
        xlim([30 65]); ylim([500 2500]);
        % add text label            
        text((max(get(gca,'XLim'))-min(get(gca,'XLim')))*0.9+min(get(gca,'XLim')),...
            (max(get(gca,'YLim'))-min(get(gca,'YLim')))*0.91+min(get(gca,'YLim')),...
    ...
    ax3=axes('position',[0.72 0.73 0.21 0.25]); hold on; grid on; % c) \Delta H
        ...
        % add colorbar
        colormap(col);
        cb1=colorbar('position',[0.935 0.77 0.012 0.2],'fontname',fontname,'fontsize',fontsize-3,'Ticks',0:1/5:1,'TickLabels',string(0:2:10));
        set(get(cb1,'title'),'String','\Deltasmr (m/a)','fontsize',fontsize-3);
    % SMB
    ax4=axes('position',[0.08 0.41 0.25 0.25]); hold on; grid on; % d) geometry
        set(gca,'fontsize',fontsize,'linewidth',2,'YTick',-1500:500:1500);
        ylabel('Elevation (m)'); 
        xlim([30 65]); ylim([-1200 800]);
        plot(x1./10^3,hb1,'-k','linewidth',2);
    ...
    ax5=axes('position',[0.405 0.41 0.25 0.25]); hold on; grid on; % e) speed
        set(gca,'fontsize',fontsize,'linewidth',2,'YTick',0:1000:5000);        
        ylabel('U (m a^{-1})');  
        xlim([30 65]); ylim([500 2500]);
        % add text label            
    ...
    ax6=axes('position',[0.72 0.41 0.21 0.25]); hold on; grid on; % f) \Delta H
        set(gca,'fontsize',fontsize,'linewidth',2);
        ylabel('smb_{max} (m a^{-1})');
        xlim([35 65]); ylim([38 50]);              
        % add text label            
        text((max(get(gca,'XLim'))-min(get(gca,'XLim')))*0.9+min(get(gca,'XLim')),...
            (max(get(gca,'YLim'))-min(get(gca,'YLim')))*0.91+min(get(gca,'YLim')),...
        ...
        % add colorbar
        colormap(col);
        cb2=colorbar('position',[0.935 0.45 0.012 0.2],'fontname',fontname,'fontsize',fontsize-3,'Ticks',0:1/5:1,'TickLabels',string(0:-2:-10));
        set(get(cb2,'title'),'String','\Deltasmb (m/a)','fontsize',fontsize-3);
    % fwd    
    ax7=axes('position',[0.08 0.09 0.25 0.25]); hold on; grid on; % g) geometry
        set(gca,'fontsize',fontsize,'linewidth',2,'YTick',-1500:500:1500);
    ...
    ax8=axes('position',[0.405 0.09 0.25 0.25]); hold on; grid on; % h) speed
        set(gca,'fontsize',fontsize,'linewidth',2,'YTick',0:1000:5000);        
    ...
    ax9=axes('position',[0.72 0.09 0.21 0.25]); hold on; grid on; % i) \Delta H
        set(gca,'fontsize',fontsize,'linewidth',2);
        xlabel('Distance Along Centerline (km)'); ylabel('fwd (m)');
        xlim([35 65]); ylim([14.5 20.5]);         
        % add text label            
        text((max(get(gca,'XLim'))-min(get(gca,'XLim')))*0.9+min(get(gca,'XLim')),...
            (max(get(gca,'YLim'))-min(get(gca,'YLim')))*0.91+min(get(gca,'YLim')),...
        ...
        % add colorbar
        colormap(col);
        cb3=colorbar('position',[0.935 0.13 0.012 0.2],'fontname',fontname,'fontsize',fontsize-3,'Ticks',0:1/2:1,'TickLabels',string(-2.5:2.5:2.5)); 
        set(get(cb3,'title'),'String','\Deltafwd (m)','fontsize',fontsize-3);    
    ...
    ax11 = axes('position',[0.39 0.18 0.27 0.75]); hold on;
        set(gca,'fontsize',fontsize,'linewidth',2); grid on;
        xlabel('smb_{max} (m a^{-1})');
        xlim([16 28]); ylim([3 4.5]);
        % add text label            
        text((max(get(gca,'XLim'))-min(get(gca,'XLim')))*0.9+min(get(gca,'XLim')),...
            (max(get(gca,'YLim'))-min(get(gca,'YLim')))*0.08+min(get(gca,'YLim')),...
            'b)','backgroundcolor','w','fontsize',fontsize,'linewidth',linewidth-1); 
...
for i=1:length(Ismr)
    ...
    % ice mass discharge
    figure(5);
    plot(ax10,files(Ismr(i)).change+1.5,Fsmr(i),'o','markersize',markersize,'linewidth',linewidth,'color',col(i,:));
    % save results 
...
T_smr = table(dsmr,dL,dgl,dH,dU,Ugl,Fsmr);
...
for i=1:length(Ismb)
    ...
    figure(4); hold on;
        ...
        dH(i) = nanmean(H2(1:c2))-nanmean(H1(1:c1)); 
        % cf and gl positions
        plot(ax6,x2(gl2)/1e3,files(Ismb(i)).change+round(max(smb0*3.1536e7)),'x',...
            'markersize',10,'linewidth',linewidth,'color',col(i,:));
        plot(ax6,x2(c2)/1e3,files(Ismb(i)).change+round(max(smb0*3.1536e7)),'o',...
            'markersize',10,'linewidth',linewidth,'color',col(i,:));        
    % ice mass discharge
    figure(5);
    plot(ax11,files(Ismb(i)).change+27,Fsmb(i),'o','markersize',markersize,'linewidth',linewidth,'color',col(i,:));
    % save results 
...
% create table to store result quantities
T_smb = table(dsmb,dL,dgl,dH,dU,Ugl,Fsmb);
...
for i=1:length(Ifwd)
    ...
    % ice mass discharge
    figure(5); 
    plot(ax12,files(Ifwd(i)).change+18,Ffwd(i),'o','markersize',markersize,'linewidth',linewidth,'color',col(i,:));    
...
% create table to store result quantities
T_fwd = table(dfwd,dL,dgl,dH,dU,Ugl,Ffwd);

% Estimate 2019 discharge
cd([homepath,'inputs-outputs']);
U19 = interp1(x0,load('Crane_centerlineSpeeds_2007-2017.mat').U(19).speed,x1); % m/s
U19_err = interp1(x0,load('Crane_centerlineSpeeds_2007-2017.mat').U(19).speed_err,x1); % m/s
H19 = H1; 
H19_err = 2641; % m (L1B bed picks + L3 surface)
W19 = interp1(x0,W0,x1);
W19_err = 15; % m
F19 = 0.95*H19(gl1)*U19(gl1)*W19(gl1)*pi*1/4*917*1e-12*3.1536e7; % Gt/a    
F19_err = F19*sqrt((H19_err/H19(gl1))^2+(nanmean(U19_err)/U19(gl1))^2+(W19_err/W19(gl1))^2)*917*1e-12*3.1536e7;% Gt/a
...
%     saveas(gcf,'fig4.1_sensitivityTests_runoff.png','png');
%     figure(5);
%     %saveas(gcf,'fig4.2_discharge.png','png');
%     saveas(gcf,'fig4.2_discharge_runoff.png','png');
