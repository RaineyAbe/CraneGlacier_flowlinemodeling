...
%       to relax model using 2009 observations. 
%   2. Tune the inner boundary flux F0 using observations of surface speed 
%       and elevation. 
%   3. Tune the fresh water depth in crevasses fwd to minimize the misfit
...
%   5. Tune the backstress sigma_b using the resistive stress term to 
...
if save_final
    cd([homepath,'inputs-outputs']);
    xi=x; dxi=dx; hi=h; hbi=hb; Wi=W; Hi=H; Ai=A; betai=beta; Ui=U; dUdxi=dUdx;
    ci=c; gli=gl; Ei=E; 
    save('Crane_flowline_100yr_output.mat','xi','dxi','hi','Wi','Hi','Ai',...
        'betai','Ui','dUdxi','ci','gli','hbi','Ei');
    disp('saved year 100 final conditions.');
end

%% 2. Tune enhancement factor E (via U & h)
...
dH_tot = NaN.*zeros(length(Efit),length(x0)); % total dH on each iteration
dH_RMSE = NaN.*zeros(length(Efit),1); % misfit of total dH
dU_RMSE = NaN.*zeros(length(Efit),1);
...
figure(10); clf
    hold on; grid on; 
    set(gca,'fontsize',14,'linewidth',2); 
    xlabel('Efit index'); ylabel('misfit (m*m/s)');
    title(['E_{best} = ',num2str(Efit(IEbest).fit(2)),'+',num2str(Efit(IEbest).fit(1)),'x']);      
    plot(1:length(Efit),misfit,'.','markersize',20);
    plot(IEbest,misfit(IEbest),'*','markersize',15,'linewidth',2);
...
%% 3. Tune fresh water depth in crevasses fwd (via c)
...
%% 4. Tune the inner boundary flux F1 (via U & h)

close all;

save_F1best = 1; % = 1 to save best F1
...
% pre-allocate misfit variables
    ...
    dH_RMSE = NaN.*zeros(length(F10),1); % misfit of total dH
    dU_RMSE = NaN.*zeros(length(F10),1);
...
for f=1:length(F10) 
    ...
    for i=1:length(t)
        ...
        F = U.*H.*W; % ice flux (m^3 s^-1)
        F(isnan(F))=0;
        F(1) = F(1)+F1; % adjust inner boundary flux
...
% calculate mean misfit
RMSE = nanmean(U_RMSE,2).*nanmean(dH_RMSE,2);

% plot results
IF1best = find(abs(RMSE)==min(abs(RMSE)),1,'first');
F1best = F10(IF1best); % m^3/s
disp(['Best F1 = ',num2str(F1best),' m^3/s']);
figure(10); clf
    set(gca,'fontsize',14,'linewidth',2); grid on; hold on;
    xlabel('F1 (m^3/s)'); ylabel('RMSE (m*m/s)');
    plot(F10,RMSE,'.','markersize',20);
    plot(F1best,RMSE(IF1best),'*','markersize',15,'linewidth',2);

% save F0best
if save_F1best
    cd([homepath,'inputs-outputs']);
    save('F1best.mat','F1best','x');
    disp('Best F1 saved.');
end

%% 5. Tune for the back pressure sigma_b (via c) 
...
for f=1:length(sigma_b0)
    ...
    for i=1:length(t)
        ...
        F = U.*H.*W; % ice flux (m^3 s^-1)
        F(isnan(F))=0;
        F(1)=F(1)+F1; % inner boundary flux (m^3 s^-1)
...
% (1) Run through the 20 model years with no change in SMB or SMR and 
%   save the output. 
% (2) Run through simulated 2009-2019, then continue evolving the model for 
%   10 years under new scenarios: \Delta(SMB) &/or \Delta(SMR) and compare 
%   changes in geometry and speed with the no change scenario.  
...
% plot results
if t(i)==t_end
    h2=h; H2=H; x2=x; c2=c; gl2=gl; U2=U; % store final geometry & speed
...
if save_final
    ...
    save(fileName,'h2','H2','c2','c2','U2','gl2','x2');
    disp('geometry saved.');
else
