...
addpath([basepath,'data/RACMO2.3/']); 
...
%% 6. Downscale Snowfall, Snowmelt, SMB, and runoff for a longer time period

close all; 

save_figure = 1;            % = 1 to save figure
save_variables = 1;         % = 1 to save final downscaled SMB
...
while loop==1
    ...
    SF.Lat = ncread([basepath,'data/RACMO2.3/RACMO2.3p2_XPEN055_snowfall_monthly_1979_2016.nc'],'lat'); % degrees north
    SF.Lon = ncread([basepath,'data/RACMO2.3/RACMO2.3p2_XPEN055_snowfall_monthly_1979_2016.nc'],'lon'); % degrees east
    SF.sf = squeeze(ncread([basepath,'data/RACMO2.3/RACMO2.3p2_XPEN055_snowfall_monthly_1979_2016.nc'],'snowfall')); % kg m-2 mo-1 
    time = ncread([basepath,'data/RACMO2.3/RACMO2.3p2_XPEN055_snowfall_monthly_1979_2016.nc'],'time'); % days since 1950/01/01
    
    SM.Lat = ncread([basepath,'data/RACMO2.3/RACMO2.3p2_XPEN055_snowmelt_monthly_1979_2016.nc'],'lat'); % degrees north
    SM.Lon = ncread([basepath,'data/RACMO2.3/RACMO2.3p2_XPEN055_snowmelt_monthly_1979_2016.nc'],'lon'); % degrees east
    SM.sm = squeeze(ncread([basepath,'data/RACMO2.3/RACMO2.3p2_XPEN055_snowmelt_monthly_1979_2016.nc'],'snowmelt')); % kg m-2 mo-1
    
    RO.Lat = ncread([basepath,'data/RACMO2.3/RACMO2.3p2_XPEN055_runoff_monthly_1979_2016.nc'],'lat'); % degrees north
    RO.Lon = ncread([basepath,'data/RACMO2.3/RACMO2.3p2_XPEN055_runoff_monthly_1979_2016.nc'],'lon'); % degrees east
    RO.ro = squeeze(ncread([basepath,'data/RACMO2.3/RACMO2.3p2_XPEN055_runoff_monthly_1979_2016.nc'],'runoff')); % kg m-2 mo-1
    
    SMB.Lat = ncread([basepath,'data/RACMO2.3/SMB_RACMO2.3p2_monthly_XPEN055_197901_201908.nc'],'lat'); % degrees north
    SMB.Lon = ncread([basepath,'data/RACMO2.3/SMB_RACMO2.3p2_monthly_XPEN055_197901_201908.nc'],'lon'); % degrees east
    SMB.smb = squeeze(ncread([basepath,'data/RACMO2.3/SMB_RACMO2.3p2_monthly_XPEN055_197901_201908.nc'],'smb')); % kg m-2 mo-1
    
    clear h; 
    h.Lat = ncread([basepath,'data/RACMO2.3/Height_latlon_XPEN055.nc'],'lat'); % degrees north
    h.Lon = ncread([basepath,'data/RACMO2.3/Height_latlon_XPEN055.nc'],'lon'); % degrees east
    h.h = ncread([basepath,'data/RACMO2.3/Height_latlon_XPEN055.nc'],'height'); % m above the surface
    h.cl = griddata(h.Lon,h.Lat,h.h,cl.Lon,cl.Lat); 
    
    T.Lat = ncread([basepath,'data/RACMO2.3/RACMO2.3p2_XPEN055_T2m_monthly_1979_2016.nc'],'lat'); % degrees north
    T.Lon = ncread([basepath,'data/RACMO2.3/RACMO2.3p2_XPEN055_T2m_monthly_1979_2016.nc'],'lon'); % degrees east
    T.T = squeeze(ncread([basepath,'data/RACMO2.3/RACMO2.3p2_XPEN055_T2m_monthly_1979_2016.nc'],'t2m')); % degrees K
    ...
    [SF.X,SF.Y] = wgs2ps(SF.Lon,SF.Lat,'StandardParallel',-71,'StandardMeridian',0);
    [SM.X,SM.Y] = wgs2ps(SM.Lon,SM.Lat,'StandardParallel',-71,'StandardMeridian',0);
    [RO.X,RO.Y] = wgs2ps(RO.Lon,RO.Lat,'StandardParallel',-71,'StandardMeridian',0);
    [SMB.X,SMB.Y] = wgs2ps(SMB.Lon,SMB.Lat,'StandardParallel',-71,'StandardMeridian',0);
...
for i=1:length(years)
    ...
    if i==1
        ...
        c.Ticks = [0 1.0]; c.TickLabels = {num2str(years(1)),num2str(years(end))};
    end 
...
% save figure
if save_figure
    saveas(gcf,[basepath,'figures/modeled_climate_variables_2009-2019.png'],'png');
    disp('Figure 1 saved');
end 
...
if save_variables
    ...
    save([basepath,'inputs-outputs/downscaled_RACMO_variables_',num2str(min(years)),'-',num2str(max(years)),'.mat'],'ro','sf','sm','smb','t','years');
...
t2 = datetime('01-Jan-1950') + days(time(end)); % final RACMO date
dt = days(time(2)) - days(time(1)); % time between each RACMO date
t = t1:(t2-t1)/(length(time)-1):t2; 
yrs = year(t1):year(t2);

% -----cumulative annual snowfall
% loop through years
SF_annual_centerline = zeros(length(yrs), 12, length(x));
for i=1:length(yrs)
    ...
    for j=1:length(Iyr)
        ...
        for k=1:length(x)
            % calculate cumulative sum of SM over each time point
            % using the mean SM at each centerline point 
            SF_annual_centerline(i,j,k) = SF.sf(RACMOx(k), RACMOy(k), Iyr(j))./800; % m/y
        end
    end 
end
% take average SF over all centerline points for each time point
SF_annual_sum = sum(SF_annual_centerline, 3, 'omitnan');
% switch to single column vector
SF_annual_sum_col = SF_annual_sum'; 
SF_annual_sum_col = SF_annual_sum_col(:);
% calculate annual cumulative sum starting in OCTOBER (beginning of melt season)
% first year
SF_annual_cumsum = zeros(1, length(SF_annual_sum_col));
SF_annual_cumsum(1:9) = cumsum(SF_annual_sum_col(1:9));
I=9; % starting index for summing
for i=1:length(yrs)-1
%     disp([num2str((i-1)*12+I+1), ' ',num2str((i-1)*12+I+12)]);
    % sum every 12 indices
    SF_annual_cumsum((i-1)*12+I+1:(i-1)*12+I+12) = cumsum(SF_annual_sum_col((i-1)*12+I+1:(i-1)*12+I+12));
end
% last year
SF_annual_cumsum(454:456) = cumsum(SF_annual_sum_col(454:456));

% -----cumulative annual snowmelt
% loop through years
SM_annual_centerline = zeros(length(yrs), 12, length(x));
for i=1:length(yrs)
    % indices of time points in that MELT year (i.e., October - March)
    Iyr = find(year(t)==yrs(i)); 
    % loop through Iyr
    for j=1:length(Iyr)
        ...
        for k=1:length(x)
            % calculate cumulative sum of SM over each time point
            % using the mean SM at each centerline point 
            SM_annual_centerline(i,j,k) = SM.sm(RACMOx(k), RACMOy(k), Iyr(j))./rho_w; % m/y
        end
    end 
end
% take average SM over all centerline points for each time point
SM_annual_sum = sum(SM_annual_centerline, 3, 'omitnan');
% switch to single column vector
SM_annual_sum_col = SM_annual_sum'; 
SM_annual_sum_col = SM_annual_sum_col(:);
% calculate annual cumulative sum starting in OCTOBER (beginning of melt season)
% first year
SM_annual_cumsum = zeros(1, length(SM_annual_sum_col));
SM_annual_cumsum(1:9) = cumsum(SM_annual_sum_col(1:9));
I=9; % starting index for summing
for i=1:length(yrs)-1
%     disp([num2str((i-1)*12+I+1), ' ',num2str((i-1)*12+I+12)]);
    % sum every 12 indices
    SM_annual_cumsum((i-1)*12+I+1:(i-1)*12+I+12) = cumsum(SM_annual_sum_col((i-1)*12+I+1:(i-1)*12+I+12));
end
% last year
...
for i=1:length(RO.linear(:,1))
    plot(axA, x/10^3, RO.diff(i,:), 'color', col(i,:),...
        'linewidth', 2);% , 'displayname', num2str(yrs(i)));
    plot(axB, years(i), mean(RO.diff(i,:),'omitnan'), '.', 'color', col(i,:),...
        'markersize', 10);%, 'displayname', num2str(years(i)));
end
...
xlabel('year');
ylabel('snowmelt [m/y]');
plot(t, SF_annual_cumsum, '-k', 'linewidth', 2);
% xlim([datetime('01-Jan-1995'), datetime('01-Jan-2016')]);
