...
h_cl_11 = load('surfaceElevationObs_1996-2018.mat').h(10).h_centerline;
...
% plot mean air temperature map
figure(1); clf; 
set(gcf,'position',[0 200 1280 400]);
...
set(gca,'fontname','Arial','fontsize',12);
plot(x/10^3,T_cl_11,'-b','linewidth',2);
grid on; 
xlabel('Distance along centerline [km]'); 
ylabel('Temperature (^oC)');
title(['Mean ',num2str(year_start),'-',num2str(year_end),' RACMO air temperature']);
hold off;
...
% plot rate factor along centerline
subplot(1,3,3); hold on; 
set(gca,'FontSize',12,'FontName','Arial'); 
plot(x/10^3,A*3.1536e7,'-m','LineWidth',3); 
grid on;
xlabel('Distance along centerline [km]'); 
ylabel('A [Pa^-^3 a^{-1}]');
title('Rate factor');
