...
save_mdot = 0; % = 1 to save resulting melt rates
...
w_b = h5read('ANT_iceshelf_melt_rates_CS2_2010-2018_v0.h5','/w_b')' ./ 3.1536e7; % [m/s]
w_b_interp = h5read('ANT_iceshelf_melt_rates_CS2_2010-2018_v0.h5','/w_b_interp')' ./ 3.1536e7; % [m/s]
w_b_uncert = h5read('ANT_iceshelf_melt_rates_CS2_2010-2018_v0.h5','/w_b_uncert')' ./ 3.1536e7; % [m/s]
...
subplot(1,2,1);  
    set(gca,'fontsize',fontsize,'linewidth',2); hold on;
    colormap(cmocean('amp'));
    imagesc(x/10^3, y/10^3, w_b*3.1536e7); grid on;
    xlabel('Easting [km]'); ylabel('Northing [km]');
    xlim([-2.4e3 -1.9e3]); ylim([0.9e3 1.4e3]);
    c=colorbar; %c.Label.String = "Submarine Melt Rate [m/yr]"; 
    caxis([0 10]);
...
subplot(1,2,2); hold on; grid on;
    ...
    xlabel('Distance Along Transect [km]'); ylabel('Submarine melt rate [m/yr]');   
...
tsxpts = NaN*ones(length(tsx(:,1)),51); tsypts = NaN*ones(length(tsy(:,1)),51); % transect points
mdot = NaN*ones(length(tsxpts(:,1)),51); X = zeros(length(tsxpts(:,1)),51); % melt rate
for i=1:length(tsx(:,1))
    ...
    subplot(1,2,1); 
    plot(tsxpts(i,:)/10^3,tsypts(i,:)/10^3,'-','linewidth',2,'color',col(i,:));
    % interpolate melt rate along transect
    mdot(i,1:length(tsxpts(i,:))) = interp2(x,y,w_b,tsxpts(i,:),tsypts(i,:)); % [m/a]
    ...
    subplot(1,2,2); 
    plot(X(i,:)/10^3, mdot(i,:)*3.1536e7, 'color', col(i,:), 'linewidth', 1);
    % calculate mean melt rate as a function of distance along transect
    if i==length(tsxpts(:,1))
        mdot_mean = mean(mdot, 'omitnan');
        % calculate a logarithmic fit
        [mdot_mean_fit, gof]= fit(X(4, ~isnan(mdot_mean))', mdot_mean(~isnan(mdot_mean))', 'exp2');
        % evaluate function with distance along transect
        mdot_mean_fit_eval = feval(mdot_mean_fit,X(4,:));
        % plot results on figure
        subplot(1,2,2); plot(X(4,:)/10^3, mdot_mean*3.1536e7, '-b', 'linewidth', 2);
        plot(X(4,:)/10^3, mdot_mean_fit_eval*3.1536e7, '--b', 'linewidth', 2);
        disp('Best fit logarithmic equation:');
        disp(['y=',num2str(round(mdot_mean_fit.a)),'*exp(',num2str(mdot_mean_fit.b),...
            '*x) + ',num2str(mdot_mean_fit.c),'*exp(',num2str(mdot_mean_fit.d),'*x)']);
    end
end
...
if save_mdot
    ...
    save('LarsenC_MeanMeltRate.mat', 'mdot', 'mdot_mean', 'X', 'tsxpts', 'tsypts',...
        'mdot_mean_fit', 'mdot_mean_fit_eval');
...
cd([basepath,'../ice_shelf_change/data/figure_1/']);
